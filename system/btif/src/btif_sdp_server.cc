...
// Accessed only on the Bluetooth main thread.
...
BtStatus remove_sdp_record(int record_id) {
  if (record_id < 0 || record_id >= MAX_SDP_SLOTS) {
    return BtifStatus(PARM_INVALID);
  }
  ...
  // Keep removal asynchronous to avoid timeout aborts when the main thread is
  // backlogged (CR 4580398). Never access sdp_slots from the caller on failure.
  return do_in_main_thread(
          base::BindOnce([](int id) { remove_sdp_record_in_main_thread(id); }, record_id));
}
