...
void sdpu_log_attribute_metrics(const RawAddress& bda, tSDP_DISCOVERY_DB* p_db) {
  ...
  for (tSDP_DISC_REC* p_rec = p_db->p_first_rec; p_rec != nullptr; p_rec = p_rec->p_next_rec) {
    ...
    bluetooth::metrics::LogMetricSdpAttribute(bda, service_uuid, 0, 0, nullptr);
    ...
    for (const auto& uuid_version_pair : uuid_version_array) {
      ...
      bluetooth::metrics::LogMetricSdpAttribute(bda, profile_uuid, ATTR_ID_BT_PROFILE_DESC_LIST,
    ...
    if (protocol_uuid != 0) {
      ...
      if (SDP_FindProtocolListElemInRec(p_rec, protocol_uuid, &protocol_elements)) {
        if (protocol_elements.num_params >= 1) {
          ...
          auto version_array = to_little_endian_array(version);
          bluetooth::metrics::LogMetricSdpAttribute(bda, protocol_uuid, ATTR_ID_PROTOCOL_DESC_LIST,
                                                    version_array.size(), version_array.data());
        }
      }
    }
    ...
    switch (service_uuid) {
      ...
      case UUID_SERVCLASS_AUDIO_SINK: {
        ...
        bluetooth::metrics::LogMetricSdpAttribute(bda, service_uuid, ATTR_ID_SUPPORTED_FEATURES,
                                                  version_array.size(), version_array.data());
        break;
      }
      ...
      case UUID_SERVCLASS_MESSAGE_ACCESS: {
        ...
        bluetooth::metrics::LogMetricSdpAttribute(bda, service_uuid, ATTR_ID_MAP_SUPPORTED_FEATURES,
      ...
      case UUID_SERVCLASS_PBAP_PSE: {
        ...
        bluetooth::metrics::LogMetricSdpAttribute(bda, service_uuid,
  ...
  if (has_di_record) {
    ...
    if (SDP_GetDiRecord(1, &di_record, p_db) == tSDP_STATUS::SDP_SUCCESS) {
      ...
      bluetooth::metrics::LogMetricSdpAttribute(bda, UUID_SERVCLASS_PNP_INFORMATION,
      ...
      bluetooth::metrics::LogMetricManufacturerInfo(
